    ...
    def __getitem__(self, index):
        ...
        if self.co_transforms_first:
            ...
            if self.has_target:
                ...
                #print(self.target_loader(self.targets[0, 0], self.base_path)[10:20, 20:30, 20:30])
                #print(self.targets[0, 0])
                '''
                target_sample = [self.target_loader(self.targets[index, i], self.base_path) for i in range(self.num_targets)]


                xshape = list(self.target_loader(self.targets[0, 0], self.base_path).shape)
                xx = to_categorical(target_sample)
                target_sample = xx.reshape(xshape + [xx.shape[-1]])
